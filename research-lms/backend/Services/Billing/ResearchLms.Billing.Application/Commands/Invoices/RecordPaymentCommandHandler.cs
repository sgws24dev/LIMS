...
public class RecordPaymentCommandHandler : IRequestHandler<RecordPaymentCommand>
{
    ...
    public RecordPaymentCommandHandler(IInvoiceRepository repository, ITenantContext tenantContext)
    {
        _repository = repository;
        _tenantContext = tenantContext;
    }

    public async Task Handle(RecordPaymentCommand request, CancellationToken ct)
    {
        var invoice = await _repository.GetByIdAsync(request.Id, ct)
            ?? throw new KeyNotFoundException($"Invoice {request.Id} not found.");

        invoice.RecordPayment(request.Amount, "system");
